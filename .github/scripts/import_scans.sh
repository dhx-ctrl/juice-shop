...
try:
    ...
    print(f'[{label}] ERROR parsing response: {e}')
    print(raw[:500])
    sys.exit(0)
# DefectDojo returns the test object on success
tid      = d.get('test', {}).get('id') if isinstance(d.get('test'), dict) else d.get('test')
found    = d.get('statistics', {}).get('after', {}) if 'statistics' in d else {}
new_f    = found.get('findings', {}).get('opened', '?')  if found else d.get('test_import_finding_action', '?')
total_f  = d.get('total_imported_findings', d.get('statistics', {}).get('after', {}).get('findings', {}).get('total', '?'))
errors   = d.get('error', d.get('message', None))
if errors:
    print(f'[{label}] ⚠ DD error: {errors}')
else:
    print(f'[{label}] ✔ test_id={tid} | total_imported={total_f} | new={new_f}')
" "$label" "$response"
}
...
echo "All scans processed successfully."print(json.dumps({
    'build_id':                   os.environ['BUILD_ID'],
    'commit_hash':                os.environ['COMMIT_HASH'],
    'branch_tag':                 os.environ['BRANCH_TAG'],
    'source_code_management_uri': os.environ['REPO_URI'],
